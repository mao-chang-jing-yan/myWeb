...
export class SchoolCollegesComponent implements OnInit {
  ...
  getData(): void {
    let url = environment["apis"]["webBase"] + environment["apis"]["QueryCollege"];
    this.loading = true;
  ...
  remove(): void {
    let url = environment["apis"]["webBase"] + environment["apis"]["DeleteCollege"];
  ...
  add(): void {
    this.router.navigateByUrl("/school/colleges/create").then(r => {
    })
  }
